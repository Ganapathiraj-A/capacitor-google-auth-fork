...
public class GoogleAuth extends Plugin {
  ...
  protected void signInResult(PluginCall call, ActivityResult result) {
    ...
    try {
      ...
      authentication.put(FIELD_ACCESS_TOKEN, null);
      authentication.put(FIELD_TOKEN_EXPIRES, null);
      authentication.put(FIELD_TOKEN_EXPIRES_IN, null);
  ...
  protected void handleOnActivityResult(int requestCode, int resultCode, Intent data) {
    if (pendingSignInCall == null || data == null) {
      return;
    }
    ...
    try {
      ...
      pendingSignInCall.resolve(user);
      pendingSignInCall = null;
    } catch (ApiException e) {
      if (SIGN_IN_CANCELLED == e.getStatusCode()) {
        pendingSignInCall.reject("The user canceled the sign-in flow.", "" + e.getStatusCode());
      } else {
        pendingSignInCall.reject("Something went wrong", "" + e.getStatusCode());
      }
    }
  }
